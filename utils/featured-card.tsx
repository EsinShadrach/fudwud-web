...
export function FeaturedItemCard({
  ...
  return (
    <div className="flex-shrink-0 w-full max-w-md overflow-hidden shadow-xl rounded-xl">
      <div className="relative">
        <div className="absolute inset-0 flex items-start justify-between p-3 bg-black/20">
          <BgBackground className="flex items-center p-1 px-2 rounded-full">
            ...
            <BgText className="text-xs opacity-50">({ratingCount - 1}+)</BgText>
          </BgBackground>
          {customerFavorite && (
...
function ProductOptionPills({ text }: { text: string }) {
  return (
    <div className="text-gray-400 bg-gray-200 rounded-md text-center max-w-[4rem] overflow-hidden p-2 text-xs text-ellipsis">
      {text}
    </div>
  );
}
